...
/// Parses JSON with // line comments and trailing commas into the same
/// Value tree as the ZON parser — so the rest of the pipeline (deserialize,
/// scene_loader, hot_reload) works unchanged.
pub const JsoncParser = struct {
    ...
    pub fn getLocation(self: *const JsoncParser) parser.Location {
        var line: usize = 1;
        var col: usize = 1;
...
// Note: JSONC uses string keys for everything — enum values become strings.
// The deserializer already handles this: deserializeEnum accepts .string values.
...
test "parse prefab in JSONC format" {
    ...
    // Enum values are strings in JSON
...
test "parse bouncing ball scene in JSONC" {
    ...
    try std.testing.expectApproxEqAbs(
        @as(f64, 400),
        @as(f64, @floatFromInt(scene.getObject("camera").?.getInteger("x").?)),
        0.001,
    );
...
test "JSONC + deserialize end-to-end" {
    // Proves JSONC feeds into the same deserializer as ZON
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    ...
    var p = JsoncParser.init(alloc,
        \\{ "x": 400, "y": 580 }
    );
    const pos = try des.deserialize(Position, try p.parse(), alloc);
    try std.testing.expectEqual(@as(i32, 400), pos.x);
    try std.testing.expectEqual(@as(i32, 580), pos.y);

    // Enum from string (JSON has no enum literals — uses strings)
    var p2 = JsoncParser.init(alloc,
        \\{ "body_type": "dynamic" }
    );
    const rb = try des.deserialize(RigidBody, try p2.parse(), alloc);
    try std.testing.expectEqual(BodyType.dynamic, rb.body_type);
}

test "JSONC + scene_loader end-to-end" {
    // Proves JSONC-parsed scenes work with the scene loader
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    var p = JsoncParser.init(alloc,
        \\{
        \\    "name": "jsonc_scene",
        \\    "scripts": ["physics", "camera"],
        \\    "camera": { "x": 400, "y": 300 },
        \\    "entities": [
        \\        { "components": { "Position": { "x": 10, "y": 20 }, "Wall": {} } },
        \\        { "prefab": "worker", "components": { "Position": { "x": 50, "y": 0 } } },
        \\    ],
        \\}
    );
    ...
    try std.testing.expectEqualStrings("jsonc_scene", scene.name);
    try std.testing.expectEqual(@as(usize, 2), scene.scripts.len);
    try std.testing.expectEqualStrings("physics", scene.scripts[0]);
    ...
    try std.testing.expect(scene.entities[0].hasComponent("Position"));
    try std.testing.expect(scene.entities[0].hasComponent("Wall"));
    try std.testing.expectEqualStrings("worker", scene.entities[1].prefab.?);
}
