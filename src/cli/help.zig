...
pub fn printHelp() void {
    std.debug.print(
        ...
        \\  bundle [dir] [--optimize=<mode>] [--output <dir>] [--build-number <n>] [--progress=<m>]  Generate + build the desktop target, then wrap the exe in a self-contained macOS `<Title>.app` (Info.plist + AppIcon.icns from `.app_icon`, the project's `assets/` staged into Contents/Resources and a launcher that runs the game from there — cli#364; default output `.labelle/<backend>_desktop/zig-out/`; `CFBundleVersion` is `<major+1>.<minor>.<patch>` of `.version` so it always increases with the release — cli#363 — unless `.build_number` in project.labelle or `--build-number <n>` (positive integer or `a.b.c`) pins it; macOS only — see cli#359)
