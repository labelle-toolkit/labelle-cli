...
pub fn printHelp() void {
    std.debug.print(
        ...
        \\  bundle [dir] [--optimize=<mode>] [--output <dir>] [--progress=<m>]  Generate + build the desktop target, then wrap the exe in a self-contained macOS `<Title>.app` (Info.plist + AppIcon.icns from `.app_icon`, the project's `assets/` staged into Contents/Resources and a launcher that runs the game from there — cli#364; default output `.labelle/<backend>_desktop/zig-out/`; macOS only — see cli#359)
