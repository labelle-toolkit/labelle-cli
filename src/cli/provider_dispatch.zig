...
fn execute(a: std.mem.Allocator, root: []const u8, cfg: project.ProjectConfig, provider: Provider, cmd: manifest.Command, lock_path: []const u8, trailing: []const []const u8) !u8 {
    ...
    const runs = try canonicalDir(a, try std.fs.path.join(a, &.{ try cache.getCacheRoot(a), "provider-runs" }));
    ...
    const packages = try canonicalDir(a, try std.fs.path.join(a, &.{ try zig_cache.globalCacheDir(a), "p" }));
