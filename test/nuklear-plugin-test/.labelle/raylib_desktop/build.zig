...
pub fn build(b: *std.Build) void {
    ...
    const plugin_debug_dep = b.dependency("labelle_debug", .{ .target = target, .optimize = optimize });
    const plugin_debug_mod = plugin_debug_dep.module("labelle_debug");
    ...
    const gui_dep = b.dependency("labelle_gui", .{ .target = target, .optimize = optimize });
    const gui_mod = gui_dep.module("gui");


    plugin_debug_mod.addImport("labelle-core", core_mod);
    plugin_debug_mod.addImport("labelle-gfx", gfx_mod);
    plugin_debug_mod.addImport("labelle-engine", engine_mod);
    plugin_debug_mod.addImport("backend_gfx", backend_gfx);
    plugin_debug_mod.addImport("backend_input", backend_input);
    plugin_debug_mod.addImport("backend_audio", backend_audio);
    plugin_debug_mod.addImport("backend_window", backend_window);
    plugin_debug_mod.addImport("gui_backend", gui_mod);
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            .imports = &.{
                ...
                .{ .name = "debug", .module = plugin_debug_mod },
    ...
    const gui_bridge_artifact = bridge_dep.artifact("nuklear_raylib_bridge");
