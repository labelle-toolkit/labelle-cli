...
pub fn setup(g: anytype) void {
    // Create a few entities for the debug inspector
    const e1 = g.createEntity();
    g.setPosition(e1, .{ .x = 100, .y = 200 });
    var shape1 = @TypeOf(g.*).ShapeComp.circle(20);
    shape1.color = .{ .r = 255, .g = 100, .b = 50, .a = 255 };
    g.addShape(e1, shape1);

    const e2 = g.createEntity();
    g.setPosition(e2, .{ .x = -100, .y = 100 });
    var shape2 = @TypeOf(g.*).ShapeComp.rectangle(40, 40);
    shape2.color = .{ .r = 50, .g = 200, .b = 100, .a = 255 };
    g.addShape(e2, shape2);

    const e3 = g.createEntity();
    g.setPosition(e3, .{ .x = 0, .y = -50 });
}
